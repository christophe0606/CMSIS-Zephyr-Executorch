...
# Script is in zephyr/scripts/, ROOT is zephyr/, APP_ROOT is executorch-example/
ROOT = Path(__file__).resolve().parent.parent
APP_ROOT = ROOT.parent
AI_LAYER = APP_ROOT / "ai_layer"
...
def collect_library_assets() -> List[Dict[str, Any]]:
    ...
    for lib_file in sorted(LIB_DIR.glob('*.a')):
        ...
        libraries.append({
            ...
            'path': str(lib_file.relative_to(APP_ROOT))
        })
...
def collect_model_assets() -> List[Dict[str, Any]]:
    ...
    if MODEL_DIR.exists():
        for model_file in sorted(MODEL_DIR.glob('*.pte')):
            ...
            models.append({
                ...
                'path': str(model_file.relative_to(APP_ROOT))
            })
    ...
    for header_file in sorted(MODEL_DIR.glob('*.h')):
        ...
        models.append({
            ...
            'path': str(header_file.relative_to(APP_ROOT))
        })
...
def collect_source_layer_info() -> Dict[str, Any]:
    ...
    for stage_name, layer_path in layer_files:
        ...
        layer_info = {
            'path': str(layer_path.relative_to(APP_ROOT)),
            'exists': True,
            'groups': [],
            'total_files': 0,
            'description': '',
